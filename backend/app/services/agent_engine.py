...
<<<<<<< HEAD
    ...
    def _persist_signal(self, event: "AgentEvent") -> None:
        """Best-effort: also write this event as a Signal row (category='agent') so
        the signal graph has durable history even though this class's own state is
        in-memory only and resets on restart. Never lets a DB error break capture --
        this class is called from hot paths across the app and must stay reliable
        even if the DB is briefly unavailable."""
        if not event.repo_name or "/" not in event.repo_name:
            return  # signals are always scoped to a repository
        try:
            from app.db.session import SessionLocal
            from app.models import Repository, Signal

            org, name = event.repo_name.split("/", 1)
            db = SessionLocal()
            try:
                ...
                db.add(Signal(
                    ...
                    severity="notable" if event.level in ("WARN", "QUOTA") else "info",
